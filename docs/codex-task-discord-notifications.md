...
**슬래시 커맨드 전용으로 전환도 이미 끝났다.** 프리픽스(`!`) 핸들러 10개와
`message_content` privileged intent, 빌트인 `!help`가 모두 제거되어 있다.
되살리지 말 것 — intent 요구가 다시 생긴다.

기준선: `189 passed, 29 skipped`.

---

## 작업 A — 알림 설정 웹훅 우선 재배치
...
4. **검증은 반드시 실행하고 결과를 보고할 것:**
   ...
   ```
   pytest 기준선은 **189 passed, 29 skipped**. 줄어들면 안 된다.
